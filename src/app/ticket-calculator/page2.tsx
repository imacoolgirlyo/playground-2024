...
class AllDiscountPolicy implements DiscountPolicy {
  constructor() {}
...
const amountDiscountPolicy = new AmountDiscountPolicy(1000);
const percentDiscountPolicy = new PercentDiscountPolicy(10);
...
interface Screening {
  ...
  getDiscountAmount(condition: DiscountCondition[]): number;
}
  ...
  const screenings: Screening[] = [
    {
      ...
      getDiscountAmount(conditions) {
        const conditionSatisfied = conditions.find((condition) =>
          condition.isSatisfiedBy(this as Screening)
        );
        ...
        return this.discountPolicy.getDiscountPrice(this.price);
      },
    },
    {
      title: "혹성탈출",
      ...
      getDiscountAmount(conditions) {
        ...
        // this.discountPolicy.getDiscountPrice
    ...
    },
    {
      ...
      sequence: 2,
      price: 20000,
      whenScreened: new Date(2024, 5, 3),
      discountPolicy: new AllDiscountPolicy(),
      getDiscountAmount(conditions) {
        const conditionSatisfied = conditions.find((condition) =>
          condition.isSatisfiedBy(this as Screening)
        );
  ...
  return (
    <div>
      ...
      <div>
        ...
        <div>할인 금액: {screeningToBuy?.getDiscountAmount(conditions)}원</div>
        <div>
          {/* 할인 후 금액: {screeningToBuy.getTotal(screeningToBuy?.price, conditions)}원 */}
        </div>
      </div>
    </div>
  );
